...
%token FOR_DEC EXT_DEF_LIST EXT_VAR_DEF EXT_STRUCT_DEF  FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE
...
Tag: ID {$$=mknode(0,STRUCT_TAG,yylineno);strcpy($$->type_id,$1);} //标识符号放在 ID
...
VarDec:  ID          {$$=mknode(0,ID,yylineno);strcpy($$->type_id,$1);}   //ID结点，标识符符号串存放结点的type_id
         | VarDec LB INT RB {$$=mknode(2,ARRAY_DEC,yylineno,$1,$3);}
         ;
/*函数名+参数定义*/
